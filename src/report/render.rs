...
fn build_json(
    ...
    let payload = serde_json::json!({
        ...
        "whole_archive_candidates": &current.whole_archive_candidates,
...
fn object_details(current: &AnalysisResult) -> String {
    ...
    let body = rows
        ...
        .map(|(target, (kind, total_size, sections))| {
            ...
            format!(
                "<tr data-search=\"{} {}\" data-kind=\"{}\"><td title=\"{}\">{}</td><td>{}</td><td>{}</td><td>{}</td><td><div class=\"hint muted\">{}</div>{}</td><td><span class=\"mono-wrap\">fwmap history trend --db history.db --metric \"object:{}\" --last 20</span></td></tr>",
                escape(&target),
                escape(&summary),
...
fn archive_details(current: &AnalysisResult) -> String {
    ...
    let body = totals
        ...
        .map(|(archive, (size, members))| {
            ...
            format!(
                "<tr data-search=\"{} {}\" data-archive=\"{}\"><td title=\"{}\">{}</td><td>{}</td><td>{}</td><td>{}</td><td><span class=\"mono-wrap\">fwmap history trend --db history.db --metric \"archive-member:{}\" --last 20</span></td></tr>",
                escape(&archive),
