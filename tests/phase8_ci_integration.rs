...
#[test]
fn cli_returns_error_exit_code_for_error_severity_rule_and_writes_markdown() {
    let dir = temp_dir("phase8");
    fs::create_dir_all(&dir).unwrap();
    let elf_path = dir.join("cpp.elf");
    let map_path = dir.join("sample.map");
    let ci_out = dir.join("summary.md");
    ...
    let code = cli::run([
        ...
        "--rules".to_string(),
        "tests/fixtures/sample_rules.toml".to_string(),
        "--ci-format".to_string(),
        "markdown".to_string(),
        "--ci-out".to_string(),
        ci_out.display().to_string(),
        "--demangle=on".to_string(),
    ])
    .unwrap();

    assert_eq!(code, 2);
    let markdown = fs::read_to_string(&ci_out).unwrap();
    assert!(markdown.contains("# fwmap CI Summary"));
    assert!(markdown.contains("forbid-g-rx-ring"));

    let _ = fs::remove_file(elf_path);
    let _ = fs::remove_file(map_path);
    let _ = fs::remove_file(ci_out);
