...
#[test]
fn analyze_auto_detects_lld_map_family() {
    let dir = temp_dir("phase10-lld");
    fs::create_dir_all(&dir).unwrap();
    let elf_path = dir.join("sample.elf");
    let map_path = dir.join("sample_lld.map");

    fs::write(&elf_path, build_cpp_elf32()).unwrap();
    fs::write(&map_path, include_str!("fixtures/sample_lld.map")).unwrap();
...
#[test]
fn cli_reports_unimplemented_toolchain_family() {
    let dir = temp_dir("phase10-unsupported");
    fs::create_dir_all(&dir).unwrap();
    let elf_path = dir.join("sample.elf");
    ...
    let err = cli::run([
        ...
        "--toolchain".to_string(),
        ToolchainSelection::Iar.to_string(),
    ])
    .unwrap_err();
