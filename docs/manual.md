...
`apps/fwmap-desktop/` には Tauri 2 + React + HeroUI ベースのデスクトップ UI があります。解析は既存の `fwmap` Rust core をそのまま使い、ローカルでの調査、比較、共有を GUI からまとめて扱えるようにしています。
...
- 実際の解析履歴は既存の `history.db` に保存しつつ、デスクトップ側の SQLite には設定、recent runs、plugin state、recent packages を保存します。
- `Runs`、`Diff`、`History`、`Regression`、`Inspector` を GUI から順に辿れます。
...
共有用の調査結果は `.fwpkg` ディレクトリ bundle として保存します。現在は zip ではなく、`manifest.json` と関連 JSON を並べた読みやすい bundle です。
...
4. 共有したいタイミングで `Packages` から package name、source context、include sections、notes、destination path を設定して bundle を作ります。
5. 作成済み package は `Packages` で reopening し、manifest、included / omitted items、plugin results を確認できます。
