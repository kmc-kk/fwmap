...
`apps/fwmap-desktop/` に Tauri 2 + React + HeroUI ベースの最小デスクトップ UI を追加しました。解析そのものは既存の `fwmap` Rust core をそのまま使い、GUI はファイル選択、ジョブ状態、直近 run の確認、設定保存に集中しています。
...
- ELF / map / rule file / Git リポジトリのパスを選んで解析を開始する
- Tauri event で `job-created` / `job-progress` / `job-finished` / `job-failed` を受け取る
- 最新ジョブの状態と直近 run 一覧を確認する
- run summary と top sections / top symbols / rule warning を確認する
- デスクトップ用 SQLite に設定と recent runs を保存する
- 解析履歴そのものは既存の `history.db` に記録する

### 現時点での制約

- `cancel job` は UI 上のプレースホルダで、解析スレッドの中断はまだ後続対応です
- 詳細チャートや密な drill-down は Phase D1 の対象外です
- GUI は CLI を置き換えるものではなく、重い core / report の確認はフロントエンドで補助する形です

## デスクトップアプリ (Phase D2)

Phase D2 では、Phase D1 の最小 desktop shell を拡張し、履歴系の解析を GUI から扱えるようにしました。解析の本体は引き続き既存の `fwmap` Rust core を使い、デスクトップ側は query / DTO / presentation の層だけを追加しています。

### 追加された画面と機能

- `Runs`: 直近 run の一覧と run detail を表示し、比較用の left / right を選べる
- `Diff`: 2 つの recorded run を比較し、ROM / RAM / warning delta と section / object / source / symbol / Rust 差分を見る
- `History`: branch / profile / target などで timeline を絞り込み、commit 単位の ROM / RAM delta を確認する
- `Range Diff`: `A..B` / `A...B` の Git range を比較し、累積 delta、worst commit、changed files 要約を見る
- `Regression`: metric / rule / entity を対象に regression origin を推定し、confidence / reasoning / evidence を見る

### 使い方の流れ

1. `Settings` で `history.db` と既定の Git リポジトリを設定します
2. `Start Analysis` から ELF / map を指定して run を記録します
3. `Runs` で比較したい 2 run を選び、`Diff` に移動して差分を見ます
4. `History` で branch や target を絞って timeline を読みます
5. `Range Diff` と `Regression` で Git-aware な履歴解析を行います

### 現時点での制約

- `cancel job` は引き続きプレースホルダで、解析スレッドの停止は未対応です
- history / range / regression は既存 `history.db` に蓄積された build が前提です
- UI ルーティングは軽量な hash state ベースで、本格的な router はまだ入れていません
- 詳細チャートや dense な drill-down 可視化は Phase D2 では未対応です

## デスクトップアプリ (Phase D3)

Phase D3 では、Desktop の Dashboard を可視化中心の画面に更新し、最新 run と履歴の状態を一画面で把握しやすくしました。CLI / core の履歴集計を backend API として追加し、その結果を Chart.js ベースのカードとグラフで表示します。

### 追加された可視化

- Overview cards: 最新 run、branch、ROM / RAM、warning、recent regression の要約
- ROM / RAM trend: 最近の analyzed build の推移を line chart で表示
- Warning pressure: warning / error の推移を line chart で表示
- Region usage: 最新 build の region 使用量を bar chart で表示
- Top growth contributors: 直近 2 build 間で増減が大きかった section / source / symbol を表示
- Recent regressions: warning を持つ build を compact card で表示
...
- overview cards
- latest run / latest history item
- recent trends
- recent regressions
- top growth sources
- region usage
...
- Dashboard の regression 一覧は軽量サマリで、完全な origin 推定 UI は `History` タブ側を使います
- chart bundle により frontend の build サイズは少し増えます
- 深い drill-down や cross-filter は今後の desktop phase で拡張予定です


## デスクトップアプリ (Phase D4)

Phase D4 では、desktop を日常的に使いやすくする foundation として、workspace / project 管理、policy editor、report export を追加しました。CLI / core を source of truth としたまま、desktop 側に永続化、編集、書き出しの層を足す構成です。

### 追加された機能

- Workspace / Project: project 名、root path、Git path、default ELF / map / debug / rule、target / profile、export dir を保存
- Active project: 現在の project を切り替え、default path を analysis や dashboard の初期値に反映
- Policy Editor: rule file を読み込み、TOML / JSON の基本 validation を行い、GUI から保存
- Report Export: dashboard / run / diff / history / regression の内容を HTML / print-friendly HTML / JSON で書き出し
- Recent exports: desktop DB に recent export を記録し、UI から見返せる
...
- policy validation は foundation としての基本 parse check で、core の全 rule semantics を完全再評価するものではありません
- export は snapshot ベースで、WYSIWYG なレポート編集や PDF ネイティブ出力は未対応です
- project aware な recent compare pair や pinned report の深い UI は次 phase で拡張予定です
