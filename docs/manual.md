...
`apps/fwmap-desktop/` には Tauri 2 + React + HeroUI ベースのデスクトップ UI があります。解析そのものは既存の `fwmap` Rust core をそのまま使い、GUI ではローカル開発で使いやすい操作導線をまとめています。
...
- デスクトップ側の SQLite に設定と recent runs を保存する
- 解析結果そのものは既存の `history.db` に記録する

### 履歴と比較

- `Runs`: 保存済み run の一覧と run detail を表示
- `Diff`: 2 つの run を比較し、ROM / RAM / warning delta と section / object / source / symbol / Rust 差分を表示
- `History`: branch / profile / target で timeline を絞り込み、commit 順の変化を確認
- `Range Diff`: `A..B` / `A...B` の Git range を比較し、累積 delta、worst commit、changed files の要約を表示
- `Regression`: metric / rule / entity の回帰起点を推定し、confidence / reasoning / evidence を表示

### 履歴の使い方

1. `Settings` で `history.db` と既定の Git リポジトリを設定します
2. `Start Analysis` から ELF / map を指定して run を保存します
3. `Runs` で比較したい 2 run を選び、`Diff` へ移動して差分を確認します
...
- Active project の切り替え
- Policy Editor
- Report Export
- Recent exports の保存と再表示
...
- Treemap / Icicle / Table の 3 つの見方
- Runs / Diff / History から Inspector へ移動する導線
- file / function 選択時の source context fallback 表示

### Inspector の使い方

1. `Runs` で run を選んで `Inspect run` を押します
2. `Diff` で 2 run を比較して `Inspect diff` を押します
3. `History` で `Inspect latest` を押します
4. Inspector で `View mode`、`Group by`、`Metric`、`Search`、`Top N` を切り替えます
5. Treemap / Icicle / Table のいずれかで項目を選びます
6. 右側の detail panel で metadata / rule hit / source context を確認します

### 現在の制約

- `cancel job` は UI 上のプレースホルダで、実際の中断はまだ未実装です
- history / range / regression / inspector は保存済み `history.db` に依存します
- policy validation は現時点では TOML / JSON の構文チェックが中心です
- export は snapshot ベースで、完全な WYSIWYG や PDF ネイティブ出力は未実装です
- regression 一覧は lightweight な summary です
- chart bundle により frontend build size はやや大きめです
- source excerpt は IDE 風の完全表示ではなく、metadata と簡易説明の fallback が中心です
- line 単位の excerpt や本格的な code viewer はまだ未実装です
- crate / dependency の source context は限定的です
- visualization は lightweight 実装で、巨大データ向けの仮想化や高度な interaction はまだ入っていません
- GUI は CLI を置き換えるものではなく、既存 core / report の表示を desktop 向けに再構成したものです
