...
- `cancel job` は UI 上のプレースホルダで、解析スレッドの中断はまだ行いません
- 詳細チャート、深い drill-down、エディタ連携は Phase D1 の対象外です
- GUI は CLI を置き換えるものではなく、既存 core / report の薄いフロントエンドです。
...
- `Runs`: 直近 run の一覧と run detail を表示し、比較対象の left / right を選べます
- `Diff`: 2 つの recorded run を比較し、ROM / RAM / warning delta と section / object / source / symbol / Rust 差分を確認できます
- `History`: branch / profile / target などの条件で timeline を読み込み、commit 単位の ROM / RAM delta を確認できます
- `Range Diff`: `A..B` や `A...B` の Git range を入力し、累積 delta、worst commit、changed files 集計を確認できます
- `Regression`: metric / rule / entity を対象に regression origin を推定し、confidence、reasoning、evidence を確認できます

### 使い方の流れ

1. `Settings` で `history.db` と既定の Git リポジトリを設定します
2. `Start Analysis` から ELF / map を解析して run を蓄積します
3. `Runs` で比較したい 2 run を選び、`Diff` に移動して差分を確認します
4. `History` で branch や target を絞って timeline を読み込みます
5. `Range Diff` や `Regression` で Git-aware な履歴分析を実行します

### 現時点の制約

- `cancel job` は引き続きプレースホルダで、解析スレッドの停止は行いません
- history / range / regression は既存 `history.db` に蓄積された build が前提です
- UI ルーティングは軽量な hash state ベースで、完全な router はまだ入れていません
