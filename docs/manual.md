...
`apps/fwmap-desktop/` ? Tauri 2 + React + HeroUI ???????????? UI ?????????????????? `fwmap` Rust core ????????GUI ???????????????? run ?????????????????
...
- ELF / map / rule file / Git ???????????????????
- Tauri event ? `job-created` / `job-progress` / `job-finished` / `job-failed` ?????
- ??????????? run ???????
- run summary ? top sections / top symbols / rule warning ?????
- ??????? SQLite ???? recent runs ?????
- ???????????? `history.db` ?????
